...
# TABLE NAMES
# ================================
LOG_TABLE = "dbo.Crypto_501_DEV_01_00_Log"
LOG_SCRIPT = "Crypto_501_DEV_00_00_Log.py"
DELETE_SCRIPT = "Crypto_501_DEV_01_11_Delete_Tables.py"
...
def setup_sql_connection():
    sql_env_file = os.path.join(SQL_ENV_DIR, "Crypto_501_sqlserver_local.env")
    if not os.path.exists(sql_env_file):
        sql_env_file = os.path.join(SQL_ENV_DIR, "Crypto_501_sqlserver_remote.env")
    if not os.path.exists(sql_env_file):
...
def log_table_exists(cursor):
    try:
        cursor.execute(
            ...
            "WHERE TABLE_NAME = 'Crypto_501_DEV_01_00_Log' AND TABLE_SCHEMA = 'dbo'"
        )
